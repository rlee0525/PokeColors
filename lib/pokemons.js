...
const POKEMON_LINKS = [
  ...
  "http://res.cloudinary.com/rlee0525/image/upload/v1487456897/45_vz1bca.svg",
...
class Pokemon {
  ...
  loadPokemon() {
    ...
    that.img.crossOrigin = 'anonymous';
      ...
      let originalWToH = this.width / this.height;
      this.height = that.canvas.height / 2.2;
      this.width = this.height * originalWToH;

  		that.ctx.drawImage(
        that.img, that.canvas.width / 2.5, that.canvas.height / 3.5,
  ...
  createBars() {
    ...
    for (var i = 1; i < 10; i++) {
      let barWidth = colorsPercentage[i].percentage * 2800;
      if (barWidth > 700) {
        barWidth = 650;
      }
      ...
      ctx.rect(0,
        ...
        `${barWidth}`, this.canvas.height / 9.4);
      ctx.stroke();
      ...
      let colorCode = document.getElementById('color-codes');
      let h = document.createElement('H1');
      let t = document.createTextNode(`${colorsPercentage[i].color}`);
      h.style.color = `${colorsPercentage[i].color}`;
      h.style.left = `${barWidth + 10}px`;
      h.style.top = `${(this.canvas.height / 9.4 + this.canvas.height / 300) *
                      (i - 1) + this.canvas.height / 16 + 25}px`;
      colorCode.append(h);
      h.appendChild(t);
    }
    ...
    let t = document.createTextNode(`${primaryColor}`);
    ...
    let t3 = document.createTextNode(`HELP!`);
    ...
    document.getElementById('search-box-input')
            .style.borderColor = `${colorsPercentage[1].color}`;
    document.getElementById('search-box-input')
            .style.color = `${colorsPercentage[0].color}`;
    document.getElementById('pikapika')
            .style.width = `${canvas.width / 3.5}px`;
    document.getElementById('random-pokemon')
            .style.backgroundColor = `${colorsPercentage[1].color}`;
  }

  searchPokemon() {
    let pokeNum = $('#search-box-input').val().toLowerCase();
  ...
  randomPokemon() {
    let pokeNum = Math.floor(Math.random() * 151);
